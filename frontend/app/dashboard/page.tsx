...
export default function DashboardPage() {
  ...
  // Prep charts data
  const deptUtilData = utilisation?.byDepartment?.map((d: any) => ({
    name: d.departmentName,
    Utilisation: d.utilisationPct,
  })) || [];

  const emptyProbData = emptyProb?.byPeriod?.map((p: any) => ({
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="premium-card p-6 bg-white dark:bg-[#1e293b]">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Departments</p>
          ...
          <div className="premium-card p-6 bg-white dark:bg-[#1e293b]">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Total Rooms</p>
                <h3 className="text-3xl font-bold mt-1 text-slate-900 dark:text-white">
          ...
          <div className="premium-card p-6 bg-white dark:bg-[#1e293b]">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Courses</p>
                <h3 className="text-3xl font-bold mt-1 text-slate-900 dark:text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="premium-card p-6 bg-gradient-to-br from-blue-600 to-indigo-700 text-white border-none">
            <div className="flex items-center gap-3">
              ...
              <h4 className="text-sm font-semibold uppercase tracking-wider opacity-80">Room Utilisation Rate</h4>
            </div>
            <div className="flex items-baseline gap-2 mt-4">
              <span className="text-5xl font-black">{utilisation?.overall || 0}%</span>
              <span className="text-xs opacity-75">overall system occupancy</span>
            </div>
            <p className="mt-4 text-xs opacity-80 leading-relaxed">
              Calculated based on currently occupied rooms versus total rooms across all active day and period slots.
            </p>
          </div>
          ...
          <div className="premium-card p-6 bg-white dark:bg-[#1e293b]">
            <div className="flex items-center gap-3">
              ...
              <h4 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Average Empty Hours</h4>
            </div>
            <div className="flex items-baseline gap-2 mt-4">
              <span className="text-5xl font-black text-slate-900 dark:text-white">{emptyHours?.averageEmptyHoursPerDay || 0} hrs</span>
              <span className="text-xs text-slate-500 dark:text-slate-400">per room / day</span>
            </div>
            <p className="mt-4 text-xs text-slate-400 leading-relaxed">
              Average daily non-scheduled hours for rooms in active departments. Lower is better.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="premium-card p-6 bg-white dark:bg-[#1e293b]">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Utilisation by Department</h3>
          ...
          <div className="premium-card p-6 bg-white dark:bg-[#1e293b]">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Empty Room Probability</h3>
            <div className="h-80 w-full">
              {emptyProbData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={emptyProbData}>
                    ...
                    <YAxis stroke="#94a3b8" fontSize={11} tickLine={false} unit="%" />
                    <Tooltip />
                    <Line type="monotone" dataKey="Probability" stroke="#6366f1" strokeWidth={2.5} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
                <div className="flex h-full items-center justify-center text-slate-400">No data available</div>
        ...
        <div className="premium-card p-6 bg-white dark:bg-[#1e293b]">
          <div className="flex items-center gap-2 mb-4 text-rose-600 dark:text-rose-400">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">Under-running Courses</h3>
          </div>
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="border-b-2 border-[var(--line-2)] text-[var(--ink-soft)] font-bold bg-[var(--canvas-2)]">
                  ...
                  <th className="py-3 px-4 whitespace-nowrap">Scheduled Hours</th>
                  <th className="py-3 px-4 text-right whitespace-nowrap">Gap (Hours)</th>
                </tr>
              </thead>
              <tbody>
                {underRunning.length > 0 ? (
                  underRunning.map((course: any) => (
                    <tr key={course.courseId} className="border-b border-[var(--line)] hover:bg-[var(--canvas-2)]/20 text-[var(--ink)]">
                      <td className="py-3.5 px-4 font-bold whitespace-nowrap">{course.courseCode}</td>
                      <td className="py-3.5 px-4">{course.courseName}</td>
                      <td className="py-3.5 px-4">{course.departmentName}</td>
                      <td className="py-3.5 px-4 font-semibold whitespace-nowrap">{course.credits}</td>
                      <td className="py-3.5 px-4 font-semibold whitespace-nowrap">{course.scheduledHours} hrs</td>
                  ...
                  <tr>
                    <td colSpan={6} className="py-6 text-center text-slate-400 italic">
                      No under-running courses flagged! All scheduled hours match credits.
                    </td>
                  </tr>
                )}
              </tbody>
